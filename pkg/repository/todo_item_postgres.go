...
func (r *TodoItemPostgres) Create(userId, itemId int, item todo_app.TodoItem) (int, error) {
	tx, err := r.db.Begin()
	if err != nil {
		return 0, err
	}
	...
	_, err = tx.Exec(createListItemsQuery, itemId, itemId)
	if err != nil {
		tx.Rollback()
		return 0, err
	}
...
func (r *TodoItemPostgres) GetAll(userId, itemId int) ([]todo_app.TodoItem, error) {
...
func (r *TodoItemPostgres) GetById(userId, itemId int) (todo_app.TodoItem, error) {
	...
	query := fmt.Sprintf(`SELECT ti.id, ti.title, ti.description, ti.done FROM %s ti INNER JOIN %s li on li.item_id = ti.id INNER JOIN %s ul on ul.list_id = li.list_id WHERE li.id = $1 AND ul.user_id = $2`, todoItemsTable, listsItemsTable, usersListsTable)
...
func (r *TodoItemPostgres) Update(userId, itemId int, input todo_app.UpdateItemInput) error {
	...
	query := fmt.Sprintf("UPDATE %s tl SET %s FROM %s li, %s ul WHERE ti.id = li.item_id AND li.list_id = ul.list_id  AND ul.user_id = $%d AND ti.id = $%d", todoListsTable, setQuery, listsItemsTable, usersListsTable, argId, argId+1)
