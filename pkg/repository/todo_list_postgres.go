...
func (r *TodoListPostgres) Create(userId int, list todo_app.TodoList) (int, error) {
	...
	tx.QueryRow(createListQuery, list.Title, list.Description)
...
func (r *TodoListPostgres) GetAll(userId int) ([]todo_app.TodoList, error) {
	...
	query := fmt.Sprintf("SELECT tl.id, tl.title, tl.description FROM %s tl INNER JOIN %s ul on tl.id = ul.list_id WHERE ul.user_id = $1", todoListsTable, usersTable)
...
func (r *TodoListPostgres) GetById(userId, listId int) (todo_app.TodoList, error) {
	...
	query := fmt.Sprintf("SELECT tl.id, tl.title, tl.description FROM %s tl INNER JOIN %s ul on tl.id = ul.list_id WHERE ul.user_id = $1 AND ul.list_id = $2", todoListsTable, usersTable)
